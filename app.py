...
def weather_analysis(weather_data): # Анализируем погодные данные
    ...
    if temperature < -20: #понизила, иначе по России всегда слишком холодно
        return "На улице слишком холодно. Самое время укутаться в одеяло и смотреть Гарри Поттера"
    if temperature > 35:
        return "На улице слишком жарко. Сделай себе лимонад и включай кондей на полную"
    if wind_speed and wind_speed > 50:
        return "Тебя снесет уроганом любви на улице. Оставайся дома с собаками"
    if humidity and humidity > 80:
        return "Это слишком влажно. The joke is up to u this time"
    if precipitation_probability > 65:
        return "Слишком большая вероятность, что там польет за окном. Самое время надеть ту самую винтажную джинсовку и взять зонтик"
    if any(condition in weather_text for condition in ["rain", "snow", "storm"]):
        return "Слишком опасно за окном. Ожидается шторм, снег или дождь. Лучше подключиться онлайн к своим запланированным встречам"

    return "Ура, ура! Время собирать рукзаки и отправляться в путешествие!"

def forecast_one(location_key): # добавила прогноз погоды на 3 день + анализ
    try:
